...
#define LIBOMX "libomxr_adapter.so"

RenesasOMXPlugin::RenesasOMXPlugin()
...
OMX_ERRORTYPE RenesasOMXPlugin::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE RenesasOMXPlugin::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    if (mLibHandle == NULL) {
        return OMX_ErrorUndefined;
    }
...
OMX_ERRORTYPE RenesasOMXPlugin::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE RenesasOMXPlugin::getRolesOfComponent(
        const char *name,
