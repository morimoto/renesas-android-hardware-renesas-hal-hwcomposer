	...
	int width, int height, bool interlace, int HZ)
{
	...
	int is_userdef_mode_presented = false;
	...
	for (i = 0; i < connector->count_modes; i++) {
		...
		if (test_mode->type & DRM_MODE_TYPE_USERDEF) {
			is_userdef_mode_presented = true;
			width = test_mode->hdisplay;
			height = test_mode->vdisplay;
			HZ = test_mode->vrefresh;
		}
	...
	for (i = 0; i < connector->count_modes; i++) {
		...
		if (!is_userdef_mode_presented &&
				(fabs(aspect - preferred_aspect_ratio) > 0.001)) {
			continue;
		}

		if (!is_userdef_mode_presented && (test->hdisplay > 1920)) {
			/* Now we don't support FullHD resolution */
			continue;
		}

		if (is_userdef_mode_presented && (test->hdisplay != width ||
